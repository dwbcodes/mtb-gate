...
| `apPassword` | **No** — exported as `***` | Must be re-entered manually after restore |
| `staPassword` | **No** — exported as `***` | Must be re-entered manually after restore |
...
2. Note down AP and station passwords (they are not in the backup).
3. Click **Factory Reset** and confirm. The device reboots with default settings.
4. Reconnect to the gate AP (`Gate-<#>-<mac>`, default password `changeme123`).
5. On the Reset page, click **Restore Config (JSON)** and select the saved file. The device restores all settings and reboots.
6. Reconnect and open **Network** → re-enter AP password and station password.
7. Re-register riders via NFC on the Riders page, or use **Peer Tools → Sync Riders** if the other gate still has the roster.
...
3. On the Reset page, click **Restore Config (JSON)** and select the start gate backup.
4. Re-enter AP and station passwords on the Network page.
5. The finish gate will auto-discover the new start gate MAC on next ping (within ~30 s) and update its `peerMac` automatically — no manual action needed on the finish gate.

**Finish gate replaced:**
1. Flash new board with current firmware.
2. Connect to new AP and restore the finish gate backup (sets `gateNumber = 12`, `triggerDelta`, wheel track, Wi-Fi channel).
3. Re-enter AP and station passwords.
4. The start gate must be updated with the new finish gate MAC via **Gate Config → Peer MAC**, or left blank so it auto-discovers on next ping.

**Intermediate gate replaced:**
1. Flash and restore intermediate gate backup (sets `gateNumber`, channel, `triggerDelta`).
2. Re-enter passwords.
3. No peer MAC changes needed (intermediate gates use broadcast pings).

## Known limitations

- **Passwords are always redacted** (`***`) in the export. This is intentional — plaintext passwords in a downloaded file are a security risk. Re-enter them manually after restore.
